...
def build_breakout_row(symbol, info, stock_hist, config):
    ...
    touch_days = int(rules["recent_touch_days"])
    dollar_volume_days = int(rules["avg_dollar_volume_days"])

    min_history = max(ma_days, exclude_days + 60, hold_days, touch_days, dollar_volume_days)
    ...
    # Old 3-year high:
    # use the full 3-year history returned by Yahoo, but EXCLUDE the most recent 30 trading days.
    ...
    recent_20_high = float(df["high"].tail(touch_days).max())
    touch_ratio = float(rules["recent_touch_ratio"])
    if recent_20_high < old_high * touch_ratio:
        return None

    recent_10 = df.tail(hold_days)
    hold_threshold = old_high * float(rules["hold_ratio"])
    hold_days_met = int((recent_10["close"] >= hold_threshold).sum())
    if hold_days_met < int(rules["hold_required_days"]):
        return None
    ...
    breakout_pct = max(0.0, distance_pct)
    return {
        "symbol": symbol,
        "company": info.get("shortName") or info.get("longName") or symbol,
        "exchange": info.get("exchangeName") or info.get("fullExchangeName") or "",
        ...
        "recent_20d_high": round(recent_20_high, 2),
