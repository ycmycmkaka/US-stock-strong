...
function getModeRows() {
  ...
  if (currentMode === "breakout") {
    return Array.isArray(dataStore.breakout_results) ? dataStore.breakout_results : [];
  }
  if (Array.isArray(dataStore.momentum_results)) return dataStore.momentum_results;
  return Array.isArray(dataStore.results) ? dataStore.results : [];
}

function getRules() {
  const rules = dataStore?.rules || {};

  // Backwards compatibility with the old results.json structure.
...
function renderSummary() {
  ...
  const momentumCount = Array.isArray(dataStore?.momentum_results)
    ...
    : (Array.isArray(dataStore?.results) ? dataStore.results.length : 0);
  const breakoutCount = Array.isArray(dataStore?.breakout_results)
    ? dataStore.breakout_results.length
    : 0;

  const label = currentMode === "breakout"
    ? "最新符合條件突破蓄勢股"
    : "最新符合條件短炒強勢股";

  const count = currentMode === "breakout" ? breakoutCount : momentumCount;
...
function renderRules() {
  ...
  if (currentMode === "breakout") {
    const excludeDays = Number(rules.exclude_recent_trading_days ?? 30);
    const bandPct = Number(rules.latest_close_band_pct ?? 5);
    const touchDays = Number(rules.recent_touch_days ?? 20);
    const touchPct = Number(rules.recent_touch_ratio ?? 0.98) * 100;
    const holdDays = Number(rules.hold_days ?? 10);
    const holdRequired = Number(rules.hold_required_days ?? 8);
    const holdPct = Number(rules.hold_ratio ?? 0.95) * 100;
    const maDays = Number(rules.ma_days ?? 200);
    const dollarDays = Number(rules.avg_dollar_volume_days ?? 10);

    const chips = [
      "只限美股",
      `市值 ≥ ${formatMarketCap(rules.market_cap_min ?? 5_000_000_000)}`,
      `最新收市 > MA${maDays}`,
      `距舊3年高位 ±${bandPct.toFixed(0)}%`,
      `${touchDays}日 High ≥ 舊頂 ${touchPct.toFixed(0)}%`,
      `${holdDays}日中 ≥ ${holdRequired}日收市企穩舊頂 ${holdPct.toFixed(0)}%`,
      `${dollarDays}日平均成交額 > ${formatDollarVolume(rules.avg_dollar_volume_min ?? 20_000_000)}`
    ];

    rulesCard.innerHTML = `
      <div class="rules-title">突破蓄勢條件</div>
      <div class="rule-chips breakout-chips">
        ${chips.map(chip => `<span class="rule-chip rule-chip-breakout">${escapeHtml(chip)}</span>`).join("")}
      </div>
      <div class="rules-extra">
        舊3年高位＝過去3年內最高 High，但排除最近 ${excludeDays} 個交易日。
      </div>
    `;
    return;
  }

  const benchmark = dataStore?.benchmark_symbol || rules.benchmark_symbol || "SPY";
  const chips = [
    "只限美股",
...
function setSortOptions() {
  if (currentMode === "breakout") {
    sortSelect.innerHTML = `
      <option value="dist_old_abs_asc">按最接近舊3年高位</option>
      <option value="hold_days_desc">按企穩日數 ↓</option>
      <option value="avg_dollar_volume_desc">按10日平均成交額 ↓</option>
      <option value="market_cap_desc">按市值 ↓</option>
      <option value="symbol_asc">按代號 A→Z</option>
    `;
    currentSort = "dist_old_abs_asc";
  } else {
    sortSelect.innerHTML = `
      <option value="rs_20d_vs_spy_pct_desc">按跑贏SPY(20D) ↓</option>
      <option value="rs_5d_vs_spy_pct_desc">按跑贏SPY(5D) ↓</option>
      <option value="dist_from_52w_high_pct_desc">按接近52週高位 ↑</option>
      <option value="market_cap_desc">按市值 ↓</option>
      <option value="symbol_asc">按代號 A→Z</option>
    `;
...
function sortRows(rows, sortKey) {
  ...
  switch (sortKey) {
    ...
    case "dist_from_52w_high_pct_desc":
      cloned.sort((a, b) => Math.abs(Number(a.dist_from_52w_high_pct) || Infinity) - Math.abs(Number(b.dist_from_52w_high_pct) || Infinity));
      break;
    case "dist_old_abs_asc":
      cloned.sort((a, b) => Math.abs(Number(a.dist_from_old_high_pct) || Infinity) - Math.abs(Number(b.dist_from_old_high_pct) || Infinity));
      break;
    case "hold_days_desc":
      cloned.sort((a, b) => (Number(b.hold_days_met) || 0) - (Number(a.hold_days_met) || 0));
      break;
    case "avg_dollar_volume_desc":
      cloned.sort((a, b) => (Number(b.avg_dollar_volume_10d) || 0) - (Number(a.avg_dollar_volume_10d) || 0));
      break;
...
function renderMomentumRows(rows) {
  return rows.map((row) => `
    <tr>
      ...
      <td><span class="badge ${marketCapBadgeClass(row.market_cap)}">${formatMarketCap(row.market_cap)}</span></td>
...
function renderBreakoutRows(rows) {
  return rows.map((row) => {
    const dist = Number(row.dist_from_old_high_pct);
    const distanceClass = Number.isFinite(dist) && Math.abs(dist) <= 2 ? "near-high" : pctClass(dist);
    ...
    return `
      <tr>
        ...
        <td><span class="badge ${marketCapBadgeClass(row.market_cap)}">${formatMarketCap(row.market_cap)}</span></td>
        <td><span class="badge badge-price">${formatPrice(row.recent_close)}</span></td>
        <td>${formatPrice(row.ma200)}</td>
        <td><span class="badge badge-old-high">${formatPrice(row.old_3y_high)}</span></td>
        <td>${escapeHtml(row.old_3y_high_date || "-")}</td>
        <td class="${distanceClass}">${signedPct(row.dist_from_old_high_pct)}</td>
        <td>${formatPrice(row.recent_20d_high)}</td>
        <td><span class="hold-badge">${escapeHtml(row.hold_days_met ?? "-")}/${escapeHtml(row.hold_days_total ?? 10)}</span></td>
        <td class="dollar-volume">${formatDollarVolume(row.avg_dollar_volume_10d)}</td>
      </tr>
    `;
  }).join("");
}
...
function renderTable(rows) {
  ...
  resultsBody.innerHTML = currentMode === "breakout"
...
function updateModeUI() {
  ...
  if (currentMode === "breakout") {
    subtitle.textContent = "突破蓄勢｜舊3年高位、MA200、高位企穩及成交額";
    tableTitle.textContent = "突破蓄勢候選股";
    tableHint.textContent = "重點：重新測試舊頂後，價格仍能在阻力附近企穩。";
  } else {
    subtitle.textContent = "短炒強勢股｜市值、5日/20日跑贏 SPY、貼近 52 週高位";
    tableTitle.textContent = "符合條件股票";
